...
**Tracked without a demo.** Two pages carry a route, a nav entry and a snapshot so drift is watched, but nothing is implemented behind them and the recorder does not touch them: `/ag2/webmcp` and `/ag2/human-in-the-loop/governed-actions`. The reason is on each route's page and in §7. Everything else under `/ag2/intelligence/` is the old `/ag2/premium/` set under a new prefix and stays in `doc-snapshot/manifest.json`'s `knownUnmapped` list.
