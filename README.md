...
| `/ag2/human-in-the-loop/governed-actions`         | `/human-in-the-loop/governed-actions`         | ✅ Working     | 🎬        | Tool-call variant. `useInterrupt` half needs a backend that pauses a run; published zod 3 schema retranslated for zod 4. |
...
| `/ag2/intelligence/quickstart`                    | `/intelligence/quickstart`                    | ⚠️ Partial    | 🎬        | Single-route transport implemented and exercised; the hosted-project steps still need `CPK_INTELLIGENCE_API_KEY`.        |
<!-- status-table:end -->
...
**Tracked without a demo.** `/ag2/webmcp` carries a route, a nav entry and a snapshot so drift is watched, but nothing is implemented behind it and the recorder does not touch it. The reason is on each route's page and in §7. Everything else under `/ag2/intelligence/` is the old `/ag2/premium/` set under a new prefix and stays in `doc-snapshot/manifest.json`'s `knownUnmapped` list.
