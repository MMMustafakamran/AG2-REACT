...
export const runThreadsHeadlessAction: PageActionHandler = async (
  ...
  // 5/5: the section the page added under the four steps — one agent per
  // thread. Two `useAgent({ agentId, runtimeAgentId, threadId })` hooks mount
  // here, each pinned to its own thread; a panel that never paints means the
  // private proxied agent did not register, which is the claim being tested.
  console.log(`   [ThreadsHeadless] 5/5: Resting on the per-thread agents...`);
