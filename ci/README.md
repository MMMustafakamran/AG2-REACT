...
├── run-name.mjs          names the run's artifacts (AG2-react-01Sep2026-0612UTC)
└── lib/
...
```
AG2-react-01Sep2026-0612UTC             ← consolidated, all clips
AG2-react-01Sep2026-0612UTC-shard-1     ← one worker's output
```
...
`write-versions.mjs` lives at `autorecorder/scripts/write-versions.mjs` rather
than here, and `automate.mjs` imports it from there. The recorder's own
`npm run doctor` needs it, and requiring a `ci/` directory to run the recorder's
doctor made the recorder un-portable — this repo was built without CI for its
first five commits and the doctor still had to pass.
