...
import { writeVersionsFile } from '../autorecorder/scripts/write-versions.mjs';
...
async function main() {
  ...
  try {
    ...
    if (busy.backend) {
      ...
      const backend = spawnServer('uv run main.py', BACKEND_DIR, 'backend.log');
    ...
    // 5. Health
    // Both servers boot at once, so both are waited on at once. Serially this
    // was the backend's wait plus the frontend's; together it is the slower one.
