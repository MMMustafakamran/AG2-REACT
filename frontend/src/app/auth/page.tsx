    ...
    <>
      ...
      <Panel title="Frontend — forwarding the token">
        ...
        <p className="mt-3 text-sm text-slate-600 dark:text-slate-400">
          This repo forwards an <code>Authorization</code> header on{" "}
          <code>CopilotKitProvider</code> instead, and only when a token is
          configured, so the app still runs unauthenticated out of the box:
        </p>
        <div className="mt-3">
      ...
      <Callout tone="warn" title="`properties` is not how the header gets there">
        <p>
          The page&apos;s frontend step is{" "}
          <code>properties=&#123;&#123; authorization: userToken &#125;&#125;</code>{" "}
          with the note &ldquo;the <code>authorization</code> property is
          forwarded to your AG2 <code>/chat</code> endpoint as a request
          header&rdquo;. <code>properties</code> is a v1{" "}
          <code>&lt;CopilotKit&gt;</code> prop that lands in the run&apos;s{" "}
          <code>forwardedProps</code> — part of the JSON body, not a header. The
          backend step then reads it as{" "}
          <code>authorization: str | None = Header(None)</code>, so the two
          halves of the page do not connect: nothing in the documented frontend
          sets the header the documented backend requires.
        </p>
        <p className="mt-2">
          The prop that does set a header is <code>headers</code>, which is what{" "}
          <code>providers.tsx</code> above uses. Flip{" "}
          <code>AUTH_BEARER_TOKEN</code> on and leave{" "}
          <code>NEXT_PUBLIC_AUTH_BEARER_TOKEN</code> unset to watch every route
          401 — that is the same failure the doc&apos;s own combination
          produces.
        </p>
      </Callout>

      <Callout tone="info" title="Two deployment sections, one snippet">
        The page splits into &ldquo;LangGraph Platform Deployment&rdquo; and
        &ldquo;Self-hosted Deployment&rdquo;. The first heading names a product
        from a different framework&apos;s docs and has nothing to do with AG2,
        and the two Python samples under them differ only in that the self-hosted
        one drops the <code>validate_your_token</code> body for a{" "}
        <code># Validate token here</code> comment. Its &ldquo;Setup Dynamic
        Agent Configuration&rdquo; heading describes nothing dynamic.
      </Callout>
